...
send      — 单点推送（按 GUID/UID，带 socketId 时回到发起请求的那条连接）
...
**`proxyRequest` 每次长连接转发都带上发起它的那条连接 id（`_sock`），与鉴权档位无关**；
业务服推消息时原样带回（yyds 的 `NewSender` 已经这么做）。网关据此认出连接换代：

| `_sock` | 会话当前连接 | 投给谁 |
|---|---|---|
| 空（主动推送：定时器 / AOI / 跨玩家） | 在线 | 当前连接 |
| 空 | 不在线 | 丢弃 |
| 有，且 == 当前连接 | — | 当前连接（最常见） |
| 有，但会话已换代，**原连接还活着** | — | **原连接**（顶号存活期内它 `Closing` 但仍可写） |
| 有，但原连接已销毁 | — | 丢弃，**绝不改投新连接** |

规则只有一条：**请求驱动的推送必须回到发起它的那条连接**。

⚠️ 这条修的是顶号时"取 ROLE 信息只收到一半"：业务服先推数据、再回确认包，
推送走 GUID（顶号后已指向新端）、确认包走请求自己的 socket（老端），
一次响应被劈成两半，两头都拿不全。带上 `_sock` 之后两者一起回到老连接——
**强制顶号模式下也成立**。

⚠️ 原连接已销毁时丢弃而不是改投：在飞包的正确处置是客户端重连时按 `handledIndex`
对账（未处理的丢弃、已处理的重登录拉权威数据），本来就不指望这一份能送达；
改投等于把上一代连接的数据发给刚上来的另一个人。
