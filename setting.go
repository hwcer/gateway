...
var Setting = struct {
	...
	//
	// 顶号是 UID 级的：目标角色已被其他在线会话占用、且新端正要让该角色落地
	//（选角回包/换角回包，见 forward 里的 negotiate）时，两种策略下**老连接的
	// 处置完全一样**：收到顶号通知，进入 cosnet.Options.SocketReplacedTime 秒的
	//"只收不发"存活期（在途回包照常送达、它自己发来的新请求被回 209），到期断开。
	// 唯一的区别是新端：
	//   true  强制顶号：新端**立即接管**角色上线（rebind→supersede 处置老会话）
	//   false 协商顶号：新端**本次登录被拒**（收到 errors.ErrReplaced + 剩余秒数），
	//         等老连接的存活期结束后再次登录即可直接上线
	//
	// 实质差别在服务器推送：强制模式下 send 按 UID 查到的已是新会话的连接，老连接
	// 那次请求的推送投给新端、确认包却走老端，一次响应被劈成两半；协商模式没有
	// 这个问题（见 resolveTarget 的代次隔离说明）。
	ForceReplace bool
}{
	C2SOAuth:     "oauth",
